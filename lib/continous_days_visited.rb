...
module ::DiscourseContinousDaysVisited
  class ContinousDaysVisited
    ...
    def days_visited_recently(time_period)
      @user.user_visits.where("visited_at > ?", time_period.days.ago.to_date).count
    end
    ...
    def days_visited_period(left, right)
      ...
      @user
        .user_visits
        .where("visited_at >= ? AND visited_at <= ?", right.days.ago.to_date, left.days.ago.to_date)
        .count
    end

    def visited_yesterday?
      @user.user_visits.find_by("visited_at = ?", 1.days.ago.to_date).present?
    end

    def visited_today?
      @user.user_visits.find_by("visited_at = ?", Time.now.to_date).present?
    end
